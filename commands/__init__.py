...
def remove_files(path, directory):
    # a - Create list with names of file in db
    filenames = format_csv(path)
    # b -Files to be deleted
    file_to_delete = diff_files(files_in_db=filenames, files_in_server=directory[0])
    file_to_delete_names = diff_files(files_in_db=filenames, files_in_server=directory[0], field_output="name")
    ...
    if confirm=="y":
        # connection.delete_files(file_to_delete)
