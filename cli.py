...
def parse_arguments(args):
    ...
    parser.add_argument("-f", "--file",
                        help="File with existant files in database")
    parser.add_argument("-p", "--path", required=True,
                        help="directory to be shown/cleaned in Dropbox")
    parser.add_argument("-k", "--key",
                        help="API Token in Dropbox",
                        required=True)
    group = parser.add_mutually_exclusive_group()
    group.add_argument("-l", "--list",
                       help="Show files", action="store_true")
...
def main(argv=None):
    ...
    try:
        directory = connection.get_files(path=args.path)  # For testing
        ...
        try:
            if args.file:
                remove_files(directory=directory,
                             filenames_to_be_kept=format_csv(args.file),
                             delete=connection.delete_files)
            else:
                raise argparse.ArgumentError(
                    args.file,
                    "File should be included to "
                    "specify files to be kept in server")
        except ConnectionError:
            print("Please, try later", file=sys.stdout)
            return 1
        else:
