...
def diff_files(*, files_in_db: Iterable[str],
               files_in_server: Iterable[Dict[str, str]],
               field_output: str = "id") -> List[str]:
    ''' Check difference between existant and files_in_db lists
        and elements not in files_in_db will be returned'''
    diff: List[str] = list()
...
def format_csv(path_csv: str) -> List[str]:
    """With first row of csv files which has a path get filename
...
class Connection:
    ...
    def _connect(self) -> Type[Dropbox]:
        ...
        try:
            ...
            print("Connection Trouble. Please,"
                  "check your internet connection!\n",
                  file=sys.stderr)
            raise
        except AuthError:
            print("Authentication Error. "
                  "Please, check if token is still valid!\n",
                  file=sys.stderr)
            raise
        else:
            return dbx
    ...
    def get_files(self, *, path: str = "") -> Tuple[
        ...
        """Return a tuples with a list of files in a given Dropbox folder path
           for first element, and list of directory for second element."""
        try:
            dbx = self._connect()
